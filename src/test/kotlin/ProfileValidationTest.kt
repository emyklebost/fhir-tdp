...
class ProfileValidationTest {
    ...
    @TestFactory
    fun createProfileValidationTests(): List<DynamicTest> {
        val file = File("src/test/resources/tests.json")
        val testCases = ConfigLoader().loadConfigOrThrow<Config>(file).testCases

        return testCases.map {
            DynamicTest.dynamicTest(it.resource) { run(it) }
        }
    }

    private fun run(input: TestCase) {
...
private fun createValidator() =
    ValidationEngine(
        ...
        context.loadFromFolder("src/test/resources/fsh-generated/resources")
    }
